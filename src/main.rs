...
use erupt_bootstrap::{
    DebugMessenger, DeviceBuilder, InstanceBuilder, QueueFamilyCriteria, Swapchain,
    SwapchainOptions, ValidationLayers,
};
...
impl TrianglePass {
    unsafe fn new(device: &DeviceLoader, surface_format: vk::SurfaceFormatKHR) -> Self {
        // Pipeline creation
        let vs_module = shader_module(device, include_bytes!(env!("raster.spv")));

        let shader_stages = [
            vk::PipelineShaderStageCreateInfoBuilder::new()
                .stage(vk::ShaderStageFlagBits::VERTEX)
                .module(vs_module)
                .name(CStr::from_ptr(cstr!("main_vs"))),
            vk::PipelineShaderStageCreateInfoBuilder::new()
                .stage(vk::ShaderStageFlagBits::FRAGMENT)
                .module(vs_module)
                .name(CStr::from_ptr(cstr!("main_fs"))),
        ];
